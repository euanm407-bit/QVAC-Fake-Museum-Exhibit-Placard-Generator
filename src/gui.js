#!/usr/bin/env node
// QVAC Fake Museum Exhibit Placard Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29381;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/museumplacard") {
      try {
        ...
        const result = await generate(modelId, object.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Museum Exhibit Placard Generator GUI ready at http://localhost:${PORT}`);
  });
